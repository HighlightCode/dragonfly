  ...
  if (tiering_enabled_) {
    ...
    if (dict_bulk_finished_) {
      ...
      if (tl_zstd_dict && CanCompressWithZstdDict(node)) {
        size_t raw_sz = node->sz;
        if (CompressNodeWithDict(node))
          malloc_size_ += ssize_t(GetLzf(node)->sz) - ssize_t(raw_sz);
      }
...
void QList::BackfillCompressWithZstdDict() {
  ...
  for (uint32_t i = 0; i < num_candidates; ++i, node = node->prev) {
    ...
    // Like everywhere else, malloc_size_ accounts for the compressed payload of a compressed node.
    size_t raw_sz = node->sz;
    if (CompressNodeWithDict(node)) {
      any_compressed = true;
      malloc_size_ += ssize_t(GetLzf(node)->sz) - ssize_t(raw_sz);
...
ssize_t QList::RecompressNode(Node* node) {
  ...
  if (IsZstdDictMode()) {
    // Never fall back to LZF here: the node may have become an edge node in the meantime, and
    // CompressRaw() does not expect the head or the tail.
    if (tl_zstd_dict && CanCompressWithZstdDict(node)) {
      size_t sz = node->sz;
      if (CompressNodeWithDict(node)) {
        node->recompress = 0;
