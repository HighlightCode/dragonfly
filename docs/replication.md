...
> **Status: DRAFT / for review.** Every claim below was checked against the code cited next to it
> (file:line references point at the state of `add-docs-for-replication` at the time of writing).
> Please correct anything that is wrong rather than assume it is right.
...
| Journal wire format / entries | `src/server/journal/types.h`, `serializer.h` |
...
[Backpressure & ACKs](#backpressure--acks) and [Takeover](#5-takeover-replttakeover--dfly-takeover))
...
`--replication_stream_output_limit` (default 64KB after flag caching) until the in-flight socket
