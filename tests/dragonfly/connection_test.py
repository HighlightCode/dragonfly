...
async def test_iobuf_shrinks_when_receive_idle(df_server: DflyInstance):
    """V2 idle connections reclaim input-buffer capacity in both aggregate and per-proactor stats."""
    if not is_resp_io_loop_v2(df_server):
    ...
    async def proactor_read_buffer_bytes():
        metrics = await df_server.metrics()
        samples = metrics["dragonfly_conn_io_buf_capacity_by_proactor"].samples
...
async def test_iobuf_shrinks_from_v1_active_path(df_server: DflyInstance):
    """V1 self-shrinks after low usage without corrupting a partially received command."""
    if is_resp_io_loop_v2(df_server):
        pytest.skip("targets V1 active IoBuf shrinking")
...
async def test_iobuf_shrinks_from_v1_complete_parse(df_server: DflyInstance):
    """V1 starts a new high-usage window before shrinking after later low-usage commands."""
    if is_resp_io_loop_v2(df_server):
        pytest.skip("targets V1 active IoBuf shrinking")
...
async def test_iobuf_high_usage_defers_v1_active_shrink(df_server: DflyInstance):
    """V1 retains capacity when the current observation window contains a large read."""
    if is_resp_io_loop_v2(df_server):
        pytest.skip("targets V1 active IoBuf shrinking")
